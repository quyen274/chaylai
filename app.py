...
if page == "Phân Tích Sản Phẩm":
    ...
    # Calculate maximum commission based on sales data
    def calculate_max_commission(sales_data):
        # Assuming product prices between 150,000 and 300,000, and commission rate between 10% and 15%
        sales = sales_data['Daily Sales'].sum()
        avg_price = 225_000  # Average of 150k and 300k
        avg_commission_rate = 0.125  # Average of 10% and 15%
        max_commission = sales * avg_price * avg_commission_rate
    ...
    if page == "Phân Tích Sản Phẩm":
        ...
        # Tính hoa hồng tối đa
        max_commission = calculate_max_commission(daily_sales)
        st.write(f"### Hoa hồng tối đa có thể đạt được trong tháng: {max_commission:,.0f} VND")
    
        # Hộp nhập liệu
        st.write("### Nhập ngân sách bạn dự định đầu tư vào Affiliate")
        user_budget = st.number_input("Ngân sách (VND):", min_value=1_000_000, max_value=35_000_000, step=500_000)
    
        # Sinh các kịch bản dựa trên ngân sách
        def generate_scenarios(base_budget, max_commission):
            scenarios = []
            for i in range(10):
                budget = base_budget - np.random.randint(0, int(base_budget * 0.2))
                scenarios.append({
                    "Ngân sách": f"{budget:,} VND",
                    "Chi tiết": {
                        "Giảm giá sản phẩm": f"{np.random.randint(5_000_000, 10_000_000):,} VND",
                        "Quảng cáo livestream": f"{np.random.randint(3_000_000, 6_000_000):,} VND",
                        "Quảng cáo bài viết": f"{np.random.randint(3_000_000, 6_000_000):,} VND",
                        "Kịch bản nội dung": f"{np.random.randint(2_000_000, 5_000_000):,} VND",
                        "Ngân sách dự phòng": f"{budget - 20_000_000:,} VND"
                    }
                })
            return scenarios
    
        if user_budget:
            scenarios = generate_scenarios(user_budget, max_commission)
    
            st.write("### Gợi ý kịch bản chiến dịch Affiliate")
            for idx, scenario in enumerate(scenarios):
                st.write(f"#### Kịch bản {idx + 1} (Ngân sách: {scenario['Ngân sách']})")
                for item, cost in scenario['Chi tiết'].items():
