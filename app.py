...
if selected == "Phân Tích Dữ Liệu":
    ...
    if not shopee_data.empty:
        ...
        future_dates = pd.date_range(shopee_data["Date"].min(), periods=len(shopee_data) + 30)

        fig, ax = plt.subplots()
        ax.plot(shopee_data["Date"], shopee_data["Average Price"], label="Actual Avg Price", color="orange")
        ax.plot(future_dates[-30:], price_prediction, "--", label="Predicted Avg Price", color="orange")
        plt.legend()
        st.pyplot(fig)
    else:
